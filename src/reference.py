...
def residuals_from_front(xs: np.ndarray, ys: np.ndarray, max_deg: Integral) -> np.ndarray:
    ...
    rhs = ys[:max_dofs] # TODO: integrate this into main matrix and maybe add another
    ...
    for base_idx in range(1, max_dofs):
        ...
        for deg in range(base_idx):
            ...
            residuals[base_idx, deg] = rhs[base_idx]**2 + residuals[base_idx-1, deg]
...
np_res = np.array([[numpy_residual(xs[:i], ys[:i], deg) for i in range(1, len(xs)+1)] for deg in range(max_deg+1)]).T
